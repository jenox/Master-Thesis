...
public struct PrEdForceApplicator: ForceApplicator {
    public init() {
    }

    public var airPressureStrength: CGFloat = 3
    public var angularResolutionStrength: CGFloat = 0.5
    public var vertexVertexRepulsionStrength: CGFloat = 25
    public var vertexEdgeRepulsionStrength: CGFloat = 10

    public func forces(in graph: VertexWeightedGraph) -> [VertexWeightedGraph.Vertex: CGVector] {
    ...
    public func applyForces(to graph: inout VertexWeightedGraph) throws {
        self.apply(self.forces(in: graph), to: &graph, edgesToCheck: graph.computeEdgesAndVerticesToCheck().0)
    }

    public func forces(in graph: PolygonalDual) -> [PolygonalDual.Vertex: CGVector] {
    ...
    public func applyForces(to graph: inout PolygonalDual) throws {
        let (edgesToCheck, verticesToCheck) = graph.storage.edgesAndVerticesToCheck
