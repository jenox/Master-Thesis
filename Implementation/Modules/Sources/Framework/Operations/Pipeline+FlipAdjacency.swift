...
extension PolygonalDual {
    public struct FlipAdjacencyOperation: Equatable, Hashable {
        public init(between u: ClusterName, and v: ClusterName) {
    ...
    public func possibleFlipAdjacencyOperations() -> Set<FlipAdjacencyOperation> {
        return Set(self.embeddedClusterGraph.flippableInternalEdges.map(FlipAdjacencyOperation.init))
    }

    public mutating func flipAdjacency(_ operation: PolygonalDual.FlipAdjacencyOperation) throws {
...
extension PolygonalDual {
    public struct CreateAdjacencyOperation: Equatable, Hashable {
        public init(over sharedNeighbor: ClusterName) {
    ...
    public func possibleCreateAdjacencyOperations() -> Set<CreateAdjacencyOperation> {
        return Set(self.embeddedClusterGraph.insertableEdges.map(CreateAdjacencyOperation.init))
    }

    public mutating func createAdjacency(_ operation: PolygonalDual.CreateAdjacencyOperation) throws {
        self.ensureValueSemantics()
...
extension PolygonalDual {
    public struct RemoveAdjacencyOperation: Equatable, Hashable {
        public init(between u: ClusterName, and v: ClusterName) {
            self.incidentFaces = [u, v]
    ...
    public func possibleRemoveAdjacencyOperations() -> Set<RemoveAdjacencyOperation> {
        return Set(self.embeddedClusterGraph.removableEdges.map(RemoveAdjacencyOperation.init))
    }

    public mutating func removeAdjacency(_ operation: PolygonalDual.RemoveAdjacencyOperation) throws {
        self.ensureValueSemantics()
...
extension MutablePolygonalDual {
    fileprivate func flipAdjacency(_ operation: PolygonalDual.FlipAdjacencyOperation) throws {
        // u = left, v = right
        // x = above, y = below
    ...
    fileprivate func createAdjacency(_ operation: PolygonalDual.CreateAdjacencyOperation) throws {
    ...
    fileprivate func removeAdjacency(_ operation: PolygonalDual.RemoveAdjacencyOperation) throws {
...
extension MutablePolygonalDual {
    ...
    private func contract(_ u: Vertex, into v: Vertex) {
        ...
        faces = Array(faces.rotated(shiftingElementToStart: faces.first(where: { $0.containsEdge(from: u, to: v) })!))
    ...
    private func expandDegenerateBoundary(at vertex: Vertex, into faceID: FaceID?) {
        ...
        faces = Array(faces.rotated(shiftingElementToStart: face))
...
extension EmbeddedClusterGraph {
    ...
    fileprivate var flippableInternalEdges: [(Vertex, Vertex)] {
    ...
    fileprivate var removableEdges: [(Vertex, Vertex)] {
    ...
    fileprivate var insertableEdges: [Vertex] {
