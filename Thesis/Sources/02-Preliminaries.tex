...
If vertices or edges lie on the boundary of the outer face, they are said to be \emph{external}, otherwise they are \emph{internal}.
...
In a \emph{(simple) contact representation} of a plane graph $G_\phi$, the vertices are represented by pairwise internally disjoint regions of the plane and edges are implicitly defined by the non-trivial contacts between the corresponding regions \cite{alam2013linear}.
