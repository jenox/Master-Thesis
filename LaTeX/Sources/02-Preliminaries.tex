...
% If $G$ is connected but not biconnected, it has a \emph{cut vertex} whose removal disconnects $G$.
...
A planar drawing $\Gamma$ of a graph $G$ partitions the plane into mututally disjoint regions called \emph{faces}.
...
Unless otherwise noted, we assume that contact representations are hole-free and that they respect the original graph $G_\phi$'s embedding, \ie{} the cyclic order of the boundaries with adjacent regions is equivalent to the cyclic order of respective incident edges for all vertices in $G_\phi$.

% If all regions are simple polygons, we call it a \emph{polygonal contact representation}.
