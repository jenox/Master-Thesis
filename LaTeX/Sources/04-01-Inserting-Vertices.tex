...
All internal faces of the filtered cluster graph \clustergraph{i} are triangles.
...
Let $p_{uvw}$ be the vertex of the map \initmap{i} that is common to faces $u$, $v$, and $w$.
...
% (in theory, when adding edges to ALL vertices on old outer face, we need to specify which edge remains on outer face)
...
Recall that the polygonal dual is a subdivision of the augmented dual $(\clustergraph{})^+$ of the cluster graph \clustergraph{}.
