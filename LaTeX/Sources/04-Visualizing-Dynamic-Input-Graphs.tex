...
This is because even though the combinatorial arrangement of regions in the map \propmap{i} is predetermined by the planar embedding of the filtered cluster graph \clustergraph{i}, the independent nature of the runs through the pipeline can result in regions with drastically different shapes and (absolute) positions.
...
This phase takes two inputs: A map \initmap{i} that the pipeline previously produced as output for some filtered cluster graph \clustergraph{i}, and a sequence of operations on said cluster graph, that, when applied to \clustergraph{i}, yields the cluster graph \clustergraph{i+1}.
The incremental transformation phase then determines how these operations translate to a polygonal dual of \clustergraph{i} and applies the translated operations to \initmap{i}, producing \initmap{i+1}, a  polygonal dual of \clustergraph{i+1}.
This new polygonal dual is then fed back into the drawing phase to make it an $\varepsilon$-area-proportional contact representation \propmap{i+1} for some small $\varepsilon > 0$ and to improve the local fatness of the map's regions.
...
This could be done by prepending an incremental clustering phase that translates changes to the simple input graph $G_i$ into changes of its filtered cluster graph \clustergraph{i}.
...
One must therefore provide the previously-produced cluster graph \clustergraph{i} as additional input to the incremental clustering phase such that it can tailor its output to the cluster graph that has already been locked in in an earlier run through the pipeline.
...
The simplest operation of all is changing a vertex $v$'s weight: We simply take the previous $\varepsilon$-proportional map \propmap{i}, update the weight of the face $f_v$ corresponding to the vertex $v$, and declare that as the new map \initmap{i+1}.
The map \initmap{i+1} then runs through the drawing phase again to account for the updated face weights.
