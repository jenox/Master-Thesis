...
A map \initmap{i} that the pipeline previously produced as output for some filtered cluster graph \clustergraph{i}, and a sequence of operations on said cluster graph, that, when applied to \clustergraph{i}, yields the cluster graph \clustergraph{i+1}.
...
We simply take the previous $\varepsilon$-proportional map \propmap{i}, update the weight of the face $f_v$ corresponding to the vertex $v$, and declare that as the new map \initmap{i+1}.
