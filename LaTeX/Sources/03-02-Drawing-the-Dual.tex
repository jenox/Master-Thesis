...
Also let $\measuredangle_{uvw} \in \lbrack -180^\circ, 180^\circ )$ denote the normalized angle from $u$ via $v$ to $w$ measured in counterclockwise direction.
Analogous to \cite{argyriou2013maximizing}, we define an angular force
%
\begin{equation}
	\vec{F}_\measuredangle(u,w;v) \coloneqq
	1 \cdot \frac{\measuredangle_{uvw} - \frac{360^\circ}{\deg(u)}}{\measuredangle_{uvw}}
	\cdot \begin{cases}
		\Norm(\Perp(\Bsc(\longvec{vu}, \longvec{vw}))) & \text{if $\measuredangle_{uvw} \neq 180^\circ$}\\
		\Norm(\longvec{uw}) & \text{otherwise.}
	\end{cases}
\end{equation}

Here $\Bsc(\vec{x},\vec{y}) \coloneqq \frac{\vec{x}}{\norm{\vec{x}}} + \frac{\vec{y}}{\norm{\vec{y}}}$ computes the bisector of its two arguments.
...
\begin{figure}[H]
	...
	\caption{Forces exerted on the successive neighbors $u$ and $w$ of $v$ where $\measuredangle_{uvw} > \frac{360^\circ}{\deg(v)}$.}
...
\begin{figure}[H]
	...
	\caption{Forces exerted on the endpoints of the edge $\{u,w\}$ and some non-incident vertex $v$ whose orthogonal projection $v_+$ onto the line through $u$ and $w$ lies on the edge.}
