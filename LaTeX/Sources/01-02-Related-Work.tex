...
Embedding and clustering are typically done independently from one another; however, the algorithms should be well-matched such that clusters form contiguous regions in the computed embedding.
...
This can be problematic, because if the clustering and the computed embedding don't fit together well, one ends up with many fragmented, noncontiguous clusters on the map.
...
Area-universal graphs are plane graphs that can realize any area assignment to its inner faces with straight-line edges.
